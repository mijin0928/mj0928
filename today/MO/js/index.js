...
function calender(){ //달력
	...
	const date = document.querySelector('.calender .wrap-title .date');
	const day = document.querySelector('.calender .wrap-title .day');
	const monthArr = [
		'December',
		'January',
		'February',
	...
	month.textContent = monthArr[m + 1];
	year.textContent = y;
	date.textContent = d;
	day.textContent = dayArr[week];
}
calender();
...
function fortune(){ //오늘의 운세
	...
	function cont(e){ //내용 보여주기
		handMove.style.left = e.clientX - 25 + 'px';
		handMove.style.top = e.clientY - 25 + 'px';
			...
			if(document.body.style.cursor !== 'default'){ 
				setTimeout(()=>{ 
					...
					setTimeout(()=>{ 
						...
						container.style.maxWidth = 768 + 'px';
					},2000)
				},2500)
			}
		})  
...
function scroll(){ 
	...
	function WInfoShow(){
		if(window.scrollY >= section.offsetTop - 100){
			weatherInfo();
			window.removeEventListener('scroll',WInfoShow);
		}
	}
	window.addEventListener('scroll',WInfoShow)
}
scroll();
