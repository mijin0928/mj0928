	...
	function preview() { //예고편
		...
		for(let i = 0; i < previewBtn.length; i++){
			previewBtn[i].addEventListener('click', (e) => {
				...
				const dimed = document.querySelectorAll('.dimed');
				for(let k = 0; k < dimed.length; k++){
					dimed[k].addEventListener('click',(e)=>{
						e.preventDefault();
						video.forEach((video)=>{
...
function slide() {//슬라이드
  ...
  const movieList = document.querySelectorAll('.list-movie > ul > li');
	const nextBtn = document.querySelector('.movie .btn-group .next-btn');
	const prevBtn = document.querySelector('.movie .btn-group .prev-btn');
	const slideWidth = movie.parentNode.clientWidth + 20;
	let count = 1;
	...
	function clone(){ //슬라이드 복사
		const firstClone = movieList[0].cloneNode(true);
		const lastClone = movieList[movieList.length - 1].cloneNode(true);
		movie.appendChild(firstClone);
		movie.insertBefore(lastClone,movie.firstChild);
	}
	
	movie.style.transform = `translateX(${-slideWidth}px)`;
	...
	function next(e){ //다음
		...
		if(count <= movieList.length + 1){
			movie.style.transform = `translateX(${-(slideWidth * count)}px)`;
			movie.style.transition = '0.5s';
		}
		if(count == movieList.length + 1){
			setTimeout(()=>{
				movie.style.transform = `translateX(${-slideWidth}px)`;
				movie.style.transition = '0s';
			},500)
			count = 1;
		}
	}
    ...
    const timerCount = setInterval(() => {
      ...
      if (time < 0) {
        ...
        setTimeout(() => {
          ...
          if(fail.classList.contains('on')){
            fail.parentNode.style.width = 100 + '%';
            fail.parentNode.style.height = 300 + 'px';
          }else{
            fail.parentNode.style.width = 0;
            fail.parentNode.style.height = 0;
          }
        }, 500)
      }
    }, 950);
            ...
            if (select[0].children[1].style.backgroundImage !== select[1].children[1].style.backgroundImage) {
              ...
              if (num == card.length / 2) {
                setTimeout(() => {
                  ...
                  if(success.classList.contains('on')){
                    success.parentNode.style.width = 100 + '%';
                    success.parentNode.style.height = 300 + 'px';
                  }else{
                    success.parentNode.style.width = 0;
                    success.parentNode.style.height = 0;
                  }
                }, 500)
                over();
                timerStop();
